...
import styles from "./un-assigned.module.scss";
export default function UnAssigned(props: any) {
  const { unAssigned, handleIsOpen, handleDelete, findRole, handleOpenAttachment, searchQuery } = props;
                ...
                <tbody>
                  {dataToDisplay?.length && dataToDisplay?.map((item: any, index: number) => (
                    <tr key={index} className="even:bg-gray-50 rounded-2xl">
                      ...
                      {/* <td className="p-3 text-gray-700">{item?.contact}</td> */}
                      ...
                      <td className="p-3 flex space-x-2">
                        ...
                        <button className="text-gray-500 hover:text-blue-500" onClick={() => handleIsOpen(item, "un-assigned")}>
